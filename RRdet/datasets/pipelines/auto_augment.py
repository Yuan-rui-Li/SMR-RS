...
from ...image import (imshear, imrotate, imtranslate, adjust_color, 
                        imequalize, adjust_brightness, adjust_contrast)
...
class Translate:
    ...
    def _translate_seg(self,
        ...
        for key in results.get('seg_fields', []):
            seg = results[key].copy()
            results[key] = imtranslate(seg, offset, direction,
...
class ColorTransform:
    ...
    def _adjust_color_img(self, results, factor=1.0):
        ...
        for key in results.get('img_fields', ['img']):
            ...
            img = results[key]
            results[key] = adjust_color(img, factor).astype(img.dtype)
...
class BrightnessTransform:
    ...
    def _adjust_brightness_img(self, results, factor=1.0):
        ...
        for key in results.get('img_fields', ['img']):
            img = results[key]
            results[key] = adjust_brightness(img,
...
class ContrastTransform:
    ...
    def _adjust_contrast_img(self, results, factor=1.0):
        ...
        for key in results.get('img_fields', ['img']):
            img = results[key]
            results[key] = adjust_contrast(img, factor).astype(img.dtype)
