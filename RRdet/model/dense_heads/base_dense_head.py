...
from ...runner import constant_init
...
class BaseDenseHead(BaseModule, metaclass=ABCMeta):
    ...
    def get_bboxes(self,
        ...
        for img_id in range(len(img_metas)):
            ...
            """
            select_single_mlvl解释:
                从由多张图片组成的batch的多阶段预测结果中取出单张图片的结果
                从[batch_levle1,batch_levle2,batch_lecle3......]
                变为[image1_level1,image1_level2,image1_level3......]
            """
            ...
            #可能是rpn_head中的_get_bboxes_single
    ...
    def _get_bboxes_single(self,
        ...
        for level_idx, (cls_score, bbox_pred, score_factor, priors) in \
            ...
            #reshape为(num_anchors,self.cls_out_channels)                                      
            cls_score = cls_score.permute(1, 2,
                                          0).reshape(-1, self.cls_out_channels)
            #在anchor_head中定义了，此处为True
            ...
            """
            """
            cfg=proposal_cfg=rpn_proposal
            根据cfg.score_thr和nms_pre预先筛选出送入nms的anchors,labels是每个anchor的类别标签
            """
            ...
            """
            self.bbox_coder在anchor_head中定义了
            self.bbox_coder.decode()函数:将预测结果解码成原图上的bbox坐标
            """
            bboxes = self.bbox_coder.decode(
                priors, bbox_pred, max_shape=img_shape)
            """
            此处的bboxes是原图上的坐标:Boxes with shape (N, num_classes * 4) or (N, 4), where 4
    ...
    def _bbox_post_process(self,
        ...
        if with_nms:
            ...
            """
            batched_nms进行非极大值抑制,原理如下:
            只输出score最高的bbox,同时剔除掉每类中和得分最高的bbox的iou(重叠程度)大于阈值的bboxes
            """
            det_bboxes, keep_idxs = batched_nms(mlvl_bboxes, mlvl_scores,
                                                mlvl_labels, cfg.nms)
            #det_bboxes有五列，前四列是坐标，第五列是类别标签
            #再进行筛选，确保每张图片的proposals不超过设定值max_per_img
    ...
    def forward_train(self,
        ...
        """
        outs = self(x)解释：
            x为feature maps, list[tensor[batch_size,chanels,H,W]......],list中的tensor为一个batch图片的单阶段特征图
            outs=(cls_score,bbox_preds):
                cls_scores (list[Tensor]): Classification scores for all
                    scale levels, each is a 4D-tensor, has shape
                    (batch_size, num_priors * num_classes, H, W).
                bbox_preds (list[Tensor]): Box energies / deltas for all
                    ...
                    (batch_size, num_priors * 4, H, W).
        """
        outs = self(x)#进入到最子类的forward函数
        if gt_labels is None:
            loss_inputs = outs + (gt_bboxes, img_metas)
        else:
            loss_inputs = outs + (gt_bboxes, gt_labels, img_metas)
        #losses=dict(loss_rpn_cls=tensor,loss_rpn_bbox=tensor),tensor中包含多个阶段的损失，每个阶段是所有图片的forward中backbone的输出阶段。
    ...
    def onnx_export(self,
        ...
        for cls_score, bbox_pred, score_factors, priors in zip(
            ...
            raise NotImplementedError("还未实现oonx功能")
