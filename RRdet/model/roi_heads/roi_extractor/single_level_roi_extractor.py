...
class SingleRoIExtractor(BaseRoIExtractor):
    ...
    def __init__(self,
                 ...
                 finest_scale=56,
                 init_cfg=None,
                 use_custom_roi_levels=False):

        #自定义参数，是否使用自定义的roi_levels
        self.use_custom_roi_levels =use_custom_roi_levels
    ...
    def map_roi_levels(self, rois, num_levels):
        ...
        """
        if not self.use_custom_roi_levels:
            scale = torch.sqrt(
                (rois[:, 3] - rois[:, 1]) * (rois[:, 4] - rois[:, 2]))
            target_lvls = torch.floor(torch.log2(scale / self.finest_scale + 1e-6))
            target_lvls = target_lvls.clamp(min=0, max=3).long()
            return target_lvls
        else:
            num_rois = rois.size()[0]
            target_lvls = torch.full((num_rois,), 0, device='cuda:0')
            print(target_lvls)
            return target_lvls
