...
def inference_detector(model, imgs, proposals=None):
    ...
    with torch.no_grad():
        # t1 = time.time()
        results, t_dict= model(return_loss=False, rescale=True, **data)

        # #计算浮点数和模型参数量
        # flops, params = profile(model, inputs=(data['img'],data['img_metas'],False))
