...
from ..utils import digit_version, load_url, mkdir_or_exist, print_log
from .dist_utils import get_dist_info

#获得当前路径的父级路径的父级路径
JSON_PATH=osp.dirname(osp.dirname(__file__))
...
def get_external_models():
    ...
    default_json_path = osp.join(JSON_PATH, 'model_zoo/open_mmlab.json')
...
def get_mmcls_models():
    mmcls_json_path = osp.join(JSON_PATH, 'model_zoo/mmcls.json')
...
def get_deprecated_model_names():
    deprecate_json_path = osp.join(JSON_PATH,
...
def save_checkpoint(model: torch.nn.Module,
    ...
    meta.update(mmcv_version='1.6.1', time=time.asctime())
