...
class DeltaXYWHBBoxCoder(BaseBBoxCoder):
    ...
    def encode(self, bboxes, gt_bboxes):
        ...
        """
        Args:
            bboxes:经sampling后被选中anchors的坐标
            gt_bboxes:经sampling后被选中的anchors对应的gt_bboxes的坐标
        """
...
def bbox2delta(proposals, gt, means=(0., 0., 0., 0.), stds=(1., 1., 1., 1.)):
    ...
    dw = torch.log(gw / pw)
    dh = torch.log(gh / ph)
    deltas = torch.stack([dx, dy, dw, dh], dim=-1)#形状为[m,4]

    means = deltas.new_tensor(means).unsqueeze(0)#unsqoeeze函数用于增加一个维度，0代表第0个维度，即最高的那个维度
    stds = deltas.new_tensor(stds).unsqueeze(0)
    deltas = deltas.sub_(means).div_(stds)#sub减，div除

    return deltas



def delta2bbox(rois,#bboxes
               deltas,#pred_bboxes
               means=(0., 0., 0., 0.),
               stds=(1., 1., 1., 1.),
               max_shape=None,
    ...
    #deltas.new_tensor():以means替换deltas中的内容,数据类型等继承deltas
    ...
    #clamp()函数:将输入input张量每个元素的夹紧到区间 [min,max]，并返回结果到一个新张量。
    else:
