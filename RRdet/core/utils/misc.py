...
def multi_apply(func, *args, **kwargs):
    ...
    """
    #partial用法到火狐浏览器收藏夹“python函数语法”查看
    pfunc = partial(func, **kwargs) if kwargs else func#选择pfunc=partial(func, **kwargs)还是func
    map_results = map(pfunc, *args)
    """
    zip(*map_results)将map_results拆分再组合,例如假设map_results结果为(a1,b1,c1);(a2,b2,c2)
    zip(*map_results)后变成(a1,a2);(b1,b2);(c1,c2)

    最后结果形式为([],[],[],...,)
    """
    return tuple(map(list, zip(*map_results)))#返回对应每个阶段featuremap的分类和坐标回归层
...
#从由多张图片组成的batch的多阶段分类结果中取出单张图片的结果
...
def filter_scores_and_topk(scores, score_thr, topk, results=None):
    ...
    #torch.nonzero:返回不为0的元素的坐标
    #得到有效元素在scores中的编号，valid_idxs是二维tensor
    valid_idxs = torch.nonzero(valid_mask)

    #topk=cfg.nms_pre
    #将propoal包含的anchor数限制在topk以内
    num_topk = min(topk, valid_idxs.size(0))
    #torch.sort is actually faster than .topk (at least on GPUs)
    #sort函数，默认dim=最后一个维度，在该维度对所有元素降序排列，并返回相对于原来位置的编号
    scores, idxs = scores.sort(descending=True)
    #获得前num_topk
    scores = scores[:num_topk]
    #得到前num_topk个anchor在scores的编号
    topk_idxs = valid_idxs[idxs[:num_topk]]
    #tensor.unbind()函数:按照dim维度方向分离tensor并返回元组,dim=1是列方向
    keep_idxs, labels = topk_idxs.unbind(dim=1)

    filtered_results = None
    if results is not None:
        if isinstance(results, dict):
            #dict.item()函数:返回每个字典项目的key和alue
            #相应的，根据keep_idxs对results中的项目进行筛选
