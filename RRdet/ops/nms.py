...
#nms(boxes_for_nms, scores, **nms_cfg_),nms_cfg_=dict(iou_threshold=0.7)
...
def batched_nms(boxes: Tensor,
    ...
    if class_agnostic:
        ...
        if boxes.size(-1) == 5:
            ...
            #tensor.to()函数:将idex的数据类型、device等属性变为和boxes一致
            offsets = idxs.to(boxes) * (
                max_coordinate + torch.tensor(1).to(boxes))
            boxes_for_nms = boxes + offsets[:, None]
    """
    假设nms_cfg_=dict(type='nms', iou_threshold=0.7),执行程序后后,有:
        nms_type='nms'
        nms_op={'iou_threshold': 0.7}
    """
    #nms_cfg_变为{iou_threshold=0.7}
    nms_type = nms_cfg_.pop('type', 'nms')
    #eval()函数查找名字为nms_type的函数或者变量并返回
    #此处nms_op=nms，即nms()函数
